...
// GLOBAL VARIABLES
//=============================================================================
// Global file pointer for log.txt
...
void init_log_file() {
    ...
    if (log_file) {
        fprintf(log_file, "=== Maze of UCSC Game Log ===\n");
        fflush(log_file);
    }
}
...
//=============================================================================
// Helper function to cap movement points at 250
static void cap_movement_points(Player *player) {
    if (player->movement_points > 250) {
        player->movement_points = 250;
    }
}

// Helper function for Bawana effects: normalize MP to 0 if negative, add bonus, then cap
static int normalize_mp_then_add(Player *player, int mp_to_add) {
...
static void log_duplicate_stairs(const Stair stairs[], int num_stairs);
static void log_duplicate_poles(const Pole poles[], int num_poles);
...
const char* format_position(int floor, int width_pos, int length_pos) {
    static char position_buffer[32];
...
int find_pole_at(Pole poles[], int num_poles, int floor, int width_pos, int length_pos) {
    for (int pole_idx = 0; pole_idx < num_poles; pole_idx++) {
        if (poles[pole_idx].w == width_pos && poles[pole_idx].l == length_pos) {
            // Compute min and max floors the pole spans
            int min_floor = (poles[pole_idx].start_floor < poles[pole_idx].end_floor) ? 
                            poles[pole_idx].start_floor : poles[pole_idx].end_floor;
            int max_floor = (poles[pole_idx].start_floor > poles[pole_idx].end_floor) ? 
                            poles[pole_idx].start_floor : poles[pole_idx].end_floor;
            
            // Check if current floor is within the pole's range
...
int check_path_validity(Player *player,
                        ...
                        int steps, int player_id __attribute__((unused)), const int flag_position[3],
    ...
    *blocking_reason = BLOCK_NONE;
    ...
    for (int step_num = 0; step_num < steps; step_num++) {
        ...
        // Check for flag capture at this step (before wall checking for next step)
        if (current_floor == flag_position[0] &&
            current_width == flag_position[1] &&
            current_length == flag_position[2]) {
            return 2; // Flag captured before any potential wall blocking
        }

        // Check for stairs and poles, then choose based on Manhattan distance
        ...
        int pole_indices_found[MAX_POLES];
        int poles_here = 0;
        for (int i = 0; i < num_poles; i++) {
            if (poles[i].start_floor == current_floor && poles[i].w == current_width && poles[i].l == current_length) {
                pole_indices_found[poles_here++] = i;
            }
        }

        int best_distance = 999999;
        int best_option_type = -1; // -1=none, 0=stair, 1=pole
        int best_stair_idx = -1;
        int best_pole_idx = -1;

        // Priority: poles first
        if (poles_here > 0) {
            for (int i = 0; i < poles_here; i++) {
                int idx = pole_indices_found[i];
                int d = manhattan_distance(poles[idx].end_floor, poles[idx].w, poles[idx].l, flag_position[0], flag_position[1], flag_position[2]);
                if (d < best_distance) {
                    best_distance = d;
                    best_option_type = 1; // pole
                    best_pole_idx = idx;
                }
            }
        } else if (stairs_here > 0) {
            for (int i = 0; i < stairs_here; i++) {
                ...
                if (d < best_distance) {
                    best_distance = d;
                    best_option_type = 0; // stair
                    best_stair_idx = stair_indices_found[i];
                }
            }
        }

        // Use stairs if it's the best option
        if (best_option_type == 0) {
            Stair *sel = &stairs[best_stair_idx];
            int df, dw, dl;
        ...
        // Use pole if it's the best option
        else if (best_option_type == 1) {
            current_floor = poles[best_pole_idx].end_floor;
            current_width = poles[best_pole_idx].w;
            current_length = poles[best_pole_idx].l;
        }
    }
...
void apply_bawana_effect(Player *player, Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH], int player_id) {
    ...
    if (current_floor != 0 || current_width < 6 || current_width > 9 || current_length < 20 || current_length > 24) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Player %c at [%d,%d,%d] not in Bawana area, no effect applied", 
                player_letter, current_floor, current_width, current_length);
        write_to_log(log_msg);
        return;
    }

    // Don't apply effect if player already has one
    if (player->bawana_effect != EFFECT_NONE) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Player %c already has Bawana effect %d, skipping", 
                player_letter, player->bawana_effect);
        write_to_log(log_msg);
        return;
    }
    ...
    // Apply effect based on cell type
    switch (cell_effect_type) {
        ...
        case BA_DISORIENTED:
            ...
            normalize_mp_then_add(player, 50);
        ...
        case BA_HAPPY:
            ...
            normalize_mp_then_add(player, 200);
            // Move to Bawana entrance
            player->pos[1] = 9; 
            player->pos[2] = 19;
            player->direction = DIR_NORTH;
        ...
        case BA_RANDOM_MP:
            ...
            normalize_mp_then_add(player, player->bawana_random_mp);
            // Move to Bawana entrance
            player->pos[1] = 9; 
            player->pos[2] = 19;
            player->direction = DIR_NORTH;
...
// Check if a position is within the Bawana area (Floor 0, w=6-9, l=20-24)
int is_in_bawana_area(int floor, int width_pos, int length_pos) {
    return (floor == 0 && width_pos >= 6 && width_pos <= 9 && length_pos >= 20 && length_pos <= 24);
}

// Handle when player falls into starting area or Bawana area via stairs/poles
void handle_fallback_to_starting_area(Player *player, int player_id, const char* transportation_type, const char* from_position) {
    char player_letter = 'A' + player_id;
    // Move player to Player A's starting position
    player->pos[0] = 0; 
    player->pos[1] = 6; 
    player->pos[2] = 12;
    player->direction = DIR_NORTH; // Same direction as Player A
    player->in_game = 0; // Must roll 6 to re-enter
    char log_msg[512];
    snprintf(log_msg, sizeof(log_msg), "WARNING: Player %c fell into starting area via %s from %s - transported to Player A's starting area [0,6,12], in_game=0, must roll 6 to re-enter", 
            player_letter, transportation_type, from_position);
    write_to_log(log_msg);
}
...
void reset_to_starting_area(Player *player, int player_id) {
    char player_letter = 'A' + player_id;
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "LOOP DETECTION: Player %c trapped in infinite loop at [%d,%d,%d], resetting to starting area", 
            player_letter, player->pos[0], player->pos[1], player->pos[2]);
    write_to_log(log_msg);
...
int move_player_with_teleport(Player *player, Player players[3], Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH],
                               Stair stairs[], int num_stairs,
                               Pole poles[], int num_poles,
    ...
    // Store starting position for flag capture logging
    int start_position[3] = {player->pos[0], player->pos[1], player->pos[2]};
    ...
    int path_result = check_path_validity(player, maze,
                                         stairs, num_stairs,
                                         poles, num_poles,
                                         walls, num_walls,
    ...
    // Handle flag capture override (result == 2)
    if (path_result == 2) {
        // Flag found at step N, execute movement only to that point
        // Execute movement step by step until flag capture
        for (int current_step = 0; current_step <= blocked_at_step; current_step++) {
            ...
            if (actual_steps_taken) {
                (*actual_steps_taken)++;
            }

            // Check for flag capture
            if (check_flag_capture(player, flag_position)) {
                char log_msg[512];
                snprintf(log_msg, sizeof(log_msg), "FLAG CAPTURE MID-MOVE: Player %c captured flag at step %d, start: %s, flag: %s, game ended early",
                        player_letter, current_step, format_position(start_position[0], start_position[1], start_position[2]),
                        format_position(flag_position[0], flag_position[1], flag_position[2]));
                write_to_log(log_msg);
                printf("Player %c has captured the flag during movement!\n", player_letter);
                printf("Player %c wins the game!\n", player_letter);
                exit(0);
            }
        }
        return 1; // Movement successful with flag capture
    }

    if (path_result == 0) {
        char log_msg[512];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Player %c movement blocked at step %d: %s (pos=[%d,%d,%d], dir=%s)", 
                player_letter, blocked_at_step, get_blockage_reason_description(reason_for_blocking),
                player->pos[0], player->pos[1], player->pos[2], get_direction_name(player->direction));
        write_to_log(log_msg);
    ...
    for (int current_step = 0; current_step < steps; current_step++) {
        ...
        // Check for flag capture after basic movement
        if (check_flag_capture(player, flag_position)) {
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "FLAG CAPTURE MID-MOVE: Player %c captured flag at step %d, start: %s, flag: %s, game ended early",
                    player_letter, current_step, format_position(start_position[0], start_position[1], start_position[2]),
        ...
        for (int history_idx = 0; history_idx < visit_counter - 1; history_idx++) {
            if (visited_positions[history_idx][0] == player->pos[0] &&
                ...
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg), "LOOP DETECTION: Infinite loop detected for Player %c at [%d,%d,%d] after basic movement", 
                        player_letter, player->pos[0], player->pos[1], player->pos[2]);
                write_to_log(log_msg);
        ...
        int poles_here = 0;
        for (int i = 0; i < num_poles; i++) {
            if (poles[i].start_floor == old_floor && poles[i].w == new_width && poles[i].l == new_length) {
                pole_indices_found[poles_here++] = i;
            }
        }
        ...
        if (poles_here > 0) {
            for (int i = 0; i < poles_here; i++) {
                int idx = pole_indices_found[i];
                int distance_to_flag = manhattan_distance(poles[idx].end_floor, poles[idx].w, poles[idx].l, flag_position[0], flag_position[1], flag_position[2]);
        ...
        if (best_option_type == 0) {
            ...
            int chosen_stair_idx = best_stair_idx;
            Stair *selected_stair = &stairs[chosen_stair_idx];
            int new_floor, new_width_dest, new_length_dest;

            // Determine the other end of the stair
            if (old_floor == selected_stair->start_floor) {
                new_floor = selected_stair->end_floor;
                new_width_dest = selected_stair->end_w;
                new_length_dest = selected_stair->end_l;
            } else {
                new_floor = selected_stair->start_floor;
                new_width_dest = selected_stair->start_w;
                new_length_dest = selected_stair->start_l;
            }

            // Move player to the other end of the stair
            player->pos[0] = new_floor;
            player->pos[1] = new_width_dest;
            player->pos[2] = new_length_dest;

            // Print stair message after teleport
            printf("%c takes the stairs and now placed at %s in floor %d.\n",
                   player_letter, format_position(new_floor, new_width_dest, new_length_dest), new_floor);

            // Check for flag capture after stair movement
            if (check_flag_capture(player, flag_position)) {
                printf("Player %c has captured the flag after using stairs!\n", player_letter);
                printf("Player %c wins the game!\n", player_letter);
                exit(0);
            }

            // Check for captures after stair movement
            for (int other_player = 0; other_player < 3; other_player++) {
                if (other_player != player_id && players[other_player].in_game) {
                    if (player->pos[0] == players[other_player].pos[0] &&
                        player->pos[1] == players[other_player].pos[1] &&
                        player->pos[2] == players[other_player].pos[2]) {
                        char other_letter = 'A' + other_player;
                        printf("Player %c captures Player %c!\n", player_letter, other_letter);
                        ...
                        // Don't return early - continue movement after teleport
                        break;
                    }
                }
            }

            // Reset visit counter and continue movement after teleport
            visit_counter = 0;
        }
        // Use pole if it's the best option
        else if (best_option_type == 1) {
            printf("%c lands on %s which is a pole cell.\n", player_letter, format_position(old_floor, new_width, new_length));
            // Move player to the end of the pole
            int new_pole_floor = poles[best_pole_idx].end_floor;
            int new_pole_width = poles[best_pole_idx].w;
            int new_pole_length = poles[best_pole_idx].l;
            player->pos[0] = new_pole_floor;
            player->pos[1] = new_pole_width;
            player->pos[2] = new_pole_length;

            // Print pole message after teleport
            printf("%c slides down and now placed at %s in floor %d.\n",
                   player_letter, format_position(new_pole_floor, new_pole_width, new_pole_length), new_pole_floor);

            // Check for flag capture after pole movement
            if (check_flag_capture(player, flag_position)) {
                printf("Player %c has captured the flag after using pole!\n", player_letter);
                printf("Player %c wins the game!\n", player_letter);
                exit(0);
            }

            // Check for captures after pole movement
            for (int other_player = 0; other_player < 3; other_player++) {
                if (other_player != player_id && players[other_player].in_game) {
                    if (player->pos[0] == players[other_player].pos[0] &&
                        player->pos[1] == players[other_player].pos[1] &&
...
void apply_movement_bonus(Player *player, Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH], int player_id) {
    ...
    switch (bonus_type) {
        case BONUS_ADD_1:
            ...
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and gains 1 movement point! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_ADD_2:
            player->movement_points += 2;
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and gains 2 movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_ADD_3:
            ...
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and gains 3 movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_ADD_4:
            ...
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and gains 4 movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_ADD_5:
            ...
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and gains 5 movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_MULTIPLY_2:
            player->movement_points *= 2;
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and doubles movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
        case BONUS_MULTIPLY_3:
            player->movement_points *= 3;
            cap_movement_points(player);
            printf("%c lands on a movement bonus cell and triples movement points! (%d -> %d)%s\n", 
                   player_letter, old_movement_points, player->movement_points,
                   player->movement_points == 250 ? " (capped at 250)" : "");
            break;
    }
...
void reset_to_bawana(Player *player, int player_id) {
    ...
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: Player %c movement points depleted, transported to Bawana at [%d,%d,%d]", 
            player_letter, player->pos[0], player->pos[1], player->pos[2]);
    write_to_log(log_msg);
}
...
void initialize_maze(Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH]) {
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: Initializing 3D maze structure: %d floors, %dx%d cells per floor", 
            NUM_FLOORS, FLOOR_WIDTH, FLOOR_LENGTH);
    write_to_log(log_msg);
    ...
    // Distribute consumable values and movement bonuses across valid cells
...
int read_seed_from_file(const char *filename) {
    ...
    if (!seed_file) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Could not open seed file %s, using current time as seed", filename);
        write_to_log(log_msg);
    ...
    if (fscanf(seed_file, "%d", &seed_value) != 1) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Could not read valid seed from %s, using current time as seed", filename);
        write_to_log(log_msg);
        fclose(seed_file);
        return time(NULL);
    }
    ...
    *num_stairs = 0;
    ...
    while (fgets(line_buffer, sizeof(line_buffer), stair_file) && *num_stairs < MAX_STAIRS) {
        ...
        if (sscanf(line_buffer, "[%d, %d, %d, %d, %d, %d]",
            ...
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "WARNING: Invalid stair format at line %d in %s", line_number, filename);
            write_to_log(log_msg);
        }
    }
    fclose(stair_file);
    // Static validation for duplicate stairs
    log_duplicate_stairs(stairs, *num_stairs);
    return 1;
}
    ...
    *num_poles = 0;
    ...
    while (fgets(line_buffer, sizeof(line_buffer), pole_file) && *num_poles < MAX_POLES) {
        ...
        if (sscanf(line_buffer, "[%d, %d, %d, %d]",
            ...
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "WARNING: Invalid pole format at line %d in %s", line_number, filename);
            write_to_log(log_msg);
        }
    }
    fclose(pole_file);
    // Static validation for duplicate poles
    log_duplicate_poles(poles, *num_poles);
    return 1;
}
    ...
    *num_walls = 0;
    ...
    while (fgets(line_buffer, sizeof(line_buffer), wall_file) && *num_walls < MAX_WALLS) {
        ...
        if (sscanf(line_buffer, "[%d, %d, %d, %d, %d]",
            ...
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "WARNING: Invalid wall format at line %d in %s", line_number, filename);
            write_to_log(log_msg);
        }
    }
    fclose(wall_file);
...
int read_flag_from_file(const char *filename, int flag_position[3]) {
    ...
    if (fgets(line_buffer, sizeof(line_buffer), flag_file)) {
        if (sscanf(line_buffer, "[%d, %d, %d]", &flag_position[0], &flag_position[1], &flag_position[2]) == 3) {
            ...
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "WARNING: Invalid flag format in %s", filename);
            write_to_log(log_msg);
        }
    } else {
        char log_msg[512];
        snprintf(log_msg, sizeof(log_msg), "WARNING: Could not read from flag file %s", filename);
        write_to_log(log_msg);
    }
    fclose(flag_file);
    return 0;
...
void update_stair_directions(Stair stairs[], int num_stairs) {
    ...
    if (total_rounds % 5 == 0) {
        ...
        char log_msg[100];
        snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: Stair directions updated after 5 rounds.");
        write_to_log(log_msg);
    }
}
...
int is_wall_blocking(Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH] __attribute__((unused)), int floor, int from_w, int from_l, int to_w, int to_l, Wall walls[], int num_walls) {
    // First, check if either position is out of bounds
    if (from_w < 0 || from_w >= FLOOR_WIDTH || from_l < 0 || from_l >= FLOOR_LENGTH ||
        to_w < 0 || to_w >= FLOOR_WIDTH || to_l < 0 || to_l >= FLOOR_LENGTH) {
    ...
    for (int wall_idx = 0; wall_idx < num_walls; wall_idx++) {
        ...
        int wall_min_w = (walls[wall_idx].start_w < walls[wall_idx].end_w) ? 
                         walls[wall_idx].start_w : walls[wall_idx].end_w;
        int wall_max_w = (walls[wall_idx].start_w > walls[wall_idx].end_w) ? 
                         walls[wall_idx].start_w : walls[wall_idx].end_w;
        int wall_min_l = (walls[wall_idx].start_l < walls[wall_idx].end_l) ? 
                         walls[wall_idx].start_l : walls[wall_idx].end_l;
        int wall_max_l = (walls[wall_idx].start_l > walls[wall_idx].end_l) ? 
                         walls[wall_idx].start_l : walls[wall_idx].end_l;

        // Vertical wall (same x-coordinate) - blocks east-west movement
        if (walls[wall_idx].start_w == walls[wall_idx].end_w) {
            int wall_w = walls[wall_idx].start_w;
            // Moving east through vertical wall (from left to right)
            if (from_w == wall_w && to_w == wall_w + 1 && 
                from_l >= wall_min_l && from_l <= wall_max_l) {
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg), "WARNING: Movement blocked by vertical wall at floor %d from [%d,%d] to [%d,%d]", floor, from_w, from_l, to_w, to_l);
                write_to_log(log_msg);
                return 1;
            }
            // Moving west through vertical wall (from right to left)
            if (from_w == wall_w + 1 && to_w == wall_w && 
                from_l >= wall_min_l && from_l <= wall_max_l) {
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg), "WARNING: Movement blocked by vertical wall at floor %d from [%d,%d] to [%d,%d]", floor, from_w, from_l, to_w, to_l);
                write_to_log(log_msg);
        ...
        // Horizontal wall (same y-coordinate) - blocks north-south movement
        if (walls[wall_idx].start_l == walls[wall_idx].end_l) {
            int wall_l = walls[wall_idx].start_l;
            // Moving south through horizontal wall (from top to bottom)
            if (from_l == wall_l && to_l == wall_l + 1 && 
                from_w >= wall_min_w && from_w <= wall_max_w) {
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg), "WARNING: Movement blocked by horizontal wall at floor %d from [%d,%d] to [%d,%d]", floor, from_w, from_l, to_w, to_l);
                write_to_log(log_msg);
                return 1;
            }
            // Moving north through horizontal wall (from bottom to top)
            if (from_l == wall_l + 1 && to_l == wall_l && 
                from_w >= wall_min_w && from_w <= wall_max_w) {
                char log_msg[256];
                snprintf(log_msg, sizeof(log_msg), "WARNING: Movement blocked by horizontal wall at floor %d from [%d,%d] to [%d,%d]", floor, from_w, from_l, to_w, to_l);
                write_to_log(log_msg);
    ...
    return 0; // No wall blocking movement
}
...
// Randomly place the flag on a valid cell in the maze
void place_random_flag(int flag_position[3], Cell maze[NUM_FLOORS][FLOOR_WIDTH][FLOOR_LENGTH]) {
    int valid_cells[NUM_FLOORS * FLOOR_WIDTH * FLOOR_LENGTH][3];
    int count = 0;
    for (int f = 0; f < NUM_FLOORS; f++) {
        for (int w = 0; w < FLOOR_WIDTH; w++) {
            for (int l = 0; l < FLOOR_LENGTH; l++) {
                if (is_valid_flag_cell(maze, f, w, l)) {
                    valid_cells[count][0] = f;
                    valid_cells[count][1] = w;
                    valid_cells[count][2] = l;
                    count++;
                }
            }
        }
    ...
    if (count > 0) {
        int idx = rand() % count;
        flag_position[0] = valid_cells[idx][0];
        flag_position[1] = valid_cells[idx][1];
        flag_position[2] = valid_cells[idx][2];
    } else {
        // Fallback: place at [0,0,0] if no valid cell found
        flag_position[0] = 0;
        flag_position[1] = 0;
        flag_position[2] = 0;
    }
...
// Check if the current player has captured any other player
void check_player_capture(Player players[3], int current_player_id) {
    Player *current = &players[current_player_id];
    for (int i = 0; i < 3; i++) {
        if (i == current_player_id) continue;
        Player *other = &players[i];
        if (other->in_game &&
            current->pos[0] == other->pos[0] &&
            current->pos[1] == other->pos[1] &&
            current->pos[2] == other->pos[2]) {
            // Capture: send other player to Player A's starting area
            other->in_game = 0;
            other->captured = 1;
            other->pos[0] = 0;
            other->pos[1] = 6;
            other->pos[2] = 12;
            other->direction = DIR_NORTH;
        }
    }
}
...
//=============================================================================
// Helper for static validation: check for duplicate stairs at same position
static void log_duplicate_stairs(const Stair stairs[], int num_stairs) {
    int duplicate_locations = 0;
    for (int i = 0; i < num_stairs; i++) {
        int count_at_start = 0;
        int count_at_end = 0;

        // Count stairs sharing the same start point
        for (int j = 0; j < num_stairs; j++) {
            if (i == j) continue;
            if (stairs[i].start_floor == stairs[j].start_floor &&
                stairs[i].start_w == stairs[j].start_w &&
                stairs[i].start_l == stairs[j].start_l) {
                count_at_start++;
            }
        }

        // Count stairs sharing the same end point
        for (int j = 0; j < num_stairs; j++) {
            if (i == j) continue;
            if (stairs[i].end_floor == stairs[j].end_floor &&
                stairs[i].end_w == stairs[j].end_w &&
                stairs[i].end_l == stairs[j].end_l) {
                count_at_end++;
            }
        }

        // Log if multiple stairs at start position
        if (count_at_start > 0) {
            char log_msg[512];
            if (count_at_start >= 2) {
                snprintf(log_msg, sizeof(log_msg), "WARNING: Multiple stairs detected at position [%d,%d,%d] - %d stairs form complete floor cycle",
                               stairs[i].start_floor, stairs[i].start_w, stairs[i].start_l, count_at_start + 1);
            } else {
                snprintf(log_msg, sizeof(log_msg), "WARNING: Multiple stairs detected at position [%d,%d,%d] - potential infinite loop risk",
        ...
        // Log if multiple stairs at end position (and not same as start)
        if (count_at_end > 0 &&
            !(stairs[i].start_floor == stairs[i].end_floor &&
              stairs[i].start_w == stairs[i].end_w &&
              stairs[i].start_l == stairs[i].end_l)) {
            char log_msg[512];
            if (count_at_end >= 2) {
                snprintf(log_msg, sizeof(log_msg), "WARNING: Multiple stairs detected at position [%d,%d,%d] - %d stairs form complete floor cycle",
                               stairs[i].end_floor, stairs[i].end_w, stairs[i].end_l, count_at_end + 1);
            } else {
                snprintf(log_msg, sizeof(log_msg), "WARNING: Multiple stairs detected at position [%d,%d,%d] - potential infinite loop risk",
                               stairs[i].end_floor, stairs[i].end_w, stairs[i].end_l);
            }
            write_to_log(log_msg);
            duplicate_locations++;
        }
    ...
    if (duplicate_locations > 0) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: Infinite loop detection enabled for complex stair configurations");
        write_to_log(log_msg);
    }
}

// Helper for static validation: check for duplicate poles at same position
static void log_duplicate_poles(const Pole poles[], int num_poles) {
    for (int i = 0; i < num_poles; i++) {
        for (int j = i + 1; j < num_poles; j++) {
            if (poles[i].w == poles[j].w && poles[i].l == poles[j].l) {
                char log_msg[512];
                snprintf(log_msg, sizeof(log_msg), "WARNING: Multiple poles detected at position [%d,%d] - floors %d->%d and %d->%d",
                               poles[i].w, poles[i].l,
                               poles[i].start_floor, poles[i].end_floor,
                               poles[j].start_floor, poles[j].end_floor);
                write_to_log(log_msg);
            }
        }
    }
    ...
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: When multiple poles present, player will slide down from highest floor first");
    write_to_log(log_msg);
}

// Helper function to determine critical cell type for logging
static const char* get_critical_cell_type(int floor, int w, int l) {
    if (floor == 0 && w == 9 && l == 19) {
        return "Bawana entrance";
    } else if (floor == 0 && w >= 6 && w <= 9 && l >= 20 && l <= 24) {
        return "Bawana interior";
    } else if ((floor == 0 && w == 5 && l == 12) ||
              (floor == 0 && w == 9 && l == 7) ||
              (floor == 0 && w == 9 && l == 17)) {
        return "spawn cell";
    } else if (floor == 0 && w >= 6 && w <= 9 && l >= 8 && l <= 16) {
        return "starting area";
    }
    return "unknown critical area";
}

// Helper function to check if a cell is critical
static int is_critical_cell(int floor, int w, int l) {
    // Bawana entrance [0,9,19]
    if (floor == 0 && w == 9 && l == 19) return 1;

    // Bawana interior [0,6-9,20-24]
    if (floor == 0 && w >= 6 && w <= 9 && l >= 20 && l <= 24) return 1;

    // Spawn cells: [0,5,12], [0,9,7], [0,9,17]
    if ((floor == 0 && w == 5 && l == 12) ||
        (floor == 0 && w == 9 && l == 7) ||
        (floor == 0 && w == 9 && l == 17)) return 1;

    // Starting area [0,6-9,8-16]
    if (floor == 0 && w >= 6 && w <= 9 && l >= 8 && l <= 16) return 1;

    return 0;
}

// Sanitize walls by trimming individual critical cells, preserving non-critical segments
void sanitize_walls(Wall walls[], int *num_walls) {
    Wall new_walls[MAX_WALLS * 4]; // Allow for wall splitting (each wall could become up to 4 segments)
    int new_wall_count = 0;
    int cells_trimmed = 0;
    int walls_processed = 0;
    int walls_kept_intact = 0;
    int wall_segments_created = 0;

    for (int wall_idx = 0; wall_idx < *num_walls; wall_idx++) {
        Wall *current_wall = &walls[wall_idx];
        walls_processed++;

        // Normalize wall coordinates
        int wall_min_w = (current_wall->start_w < current_wall->end_w) ? current_wall->start_w : current_wall->end_w;
        int wall_max_w = (current_wall->start_w > current_wall->end_w) ? current_wall->start_w : current_wall->end_w;
        int wall_min_l = (current_wall->start_l < current_wall->end_l) ? current_wall->start_l : current_wall->end_l;
        int wall_max_l = (current_wall->start_l > current_wall->end_l) ? current_wall->start_l : current_wall->end_l;

        // Determine wall type and orientation
        int is_horizontal = (wall_min_l == wall_max_l); // Same L coordinate = horizontal wall
        int is_vertical = (wall_min_w == wall_max_w);   // Same W coordinate = vertical wall

        if (!is_horizontal && !is_vertical) {
            // Diagonal wall - keep as is for now (complex case)
            char log_msg[512];
            snprintf(log_msg, sizeof(log_msg), "ASSUMPTION: Diagonal wall [%d,%d,%d]-[%d,%d,%d] kept intact (diagonal wall trimming not implemented)",
                    current_wall->floor, current_wall->start_w, current_wall->start_l,
                    current_wall->floor, current_wall->end_w, current_wall->end_l);
            write_to_log(log_msg);
            new_walls[new_wall_count++] = *current_wall;
            walls_kept_intact++;
            continue;
        }

        // Find all critical cells that intersect with this wall
        int critical_positions[100]; // Store positions of critical cells along wall
        int num_critical_positions = 0;

        if (is_horizontal) {
            // Horizontal wall: check each width position along the wall
            for (int w = wall_min_w; w <= wall_max_w; w++) {
                if (is_critical_cell(current_wall->floor, w, wall_min_l)) {
                    critical_positions[num_critical_positions++] = w;
                    cells_trimmed++;

                    char log_msg[512];
                    snprintf(log_msg, sizeof(log_msg), "TRIM: Cell [%d,%d,%d] removed from wall - overlaps %s",
                            current_wall->floor, w, wall_min_l, get_critical_cell_type(current_wall->floor, w, wall_min_l));
                    write_to_log(log_msg);
                }
            }
        } else { // is_vertical
            // Vertical wall: check each length position along the wall
            for (int l = wall_min_l; l <= wall_max_l; l++) {
                if (is_critical_cell(current_wall->floor, wall_min_w, l)) {
                    critical_positions[num_critical_positions++] = l;
                    cells_trimmed++;

                    char log_msg[512];
                    snprintf(log_msg, sizeof(log_msg), "TRIM: Cell [%d,%d,%d] removed from wall - overlaps %s",
                            current_wall->floor, wall_min_w, l, get_critical_cell_type(current_wall->floor, wall_min_w, l));
                    write_to_log(log_msg);
                }
            }
        }

        if (num_critical_positions == 0) {
            // No critical cells found, keep wall intact
            new_walls[new_wall_count++] = *current_wall;
            walls_kept_intact++;
            continue;
        }

        // Split the wall around critical positions
        if (is_horizontal) {
            // Process horizontal wall segments
            int last_pos = wall_min_w - 1;

            for (int i = 0; i < num_critical_positions; i++) {
                int critical_pos = critical_positions[i];

                // Add segment before critical position
                if (critical_pos > last_pos + 1) {
                    Wall segment = {current_wall->floor, last_pos + 1, wall_min_l, critical_pos - 1, wall_min_l};
                    new_walls[new_wall_count++] = segment;
                    wall_segments_created++;

                    char log_msg[512];
                    snprintf(log_msg, sizeof(log_msg), "SEGMENT: Created wall segment [%d,%d,%d]-[%d,%d,%d] (before critical cell)",
                            segment.floor, segment.start_w, segment.start_l, segment.floor, segment.end_w, segment.end_l);
                    write_to_log(log_msg);
                }
                last_pos = critical_pos;
            }

            // Add final segment after last critical position
            if (wall_max_w > last_pos) {
                Wall segment = {current_wall->floor, last_pos + 1, wall_min_l, wall_max_w, wall_min_l};
                new_walls[new_wall_count++] = segment;
                wall_segments_created++;

                char log_msg[512];
                snprintf(log_msg, sizeof(log_msg), "SEGMENT: Created wall segment [%d,%d,%d]-[%d,%d,%d] (after critical cell)",
                        segment.floor, segment.start_w, segment.start_l, segment.floor, segment.end_w, segment.end_l);
                write_to_log(log_msg);
            }
        } else { // is_vertical
            // Process vertical wall segments
            int last_pos = wall_min_l - 1;
            ...
            for (int i = 0; i < num_critical_positions; i++) {
                ...
                // Add segment before critical position
                if (critical_pos > last_pos + 1) {
                    Wall segment = {current_wall->floor, wall_min_w, last_pos + 1, wall_min_w, critical_pos - 1};
                    new_walls[new_wall_count++] = segment;
                    wall_segments_created++;
            ...
            // Add final segment after last critical position
            if (wall_max_l > last_pos) {
                Wall segment = {current_wall->floor, wall_min_w, last_pos + 1, wall_min_w, wall_max_l};
                new_walls[new_wall_count++] = segment;
                wall_segments_created++;
    ...
    // Replace original walls with new trimmed walls
    for (int i = 0; i < new_wall_count && i < MAX_WALLS; i++) {
        walls[i] = new_walls[i];
    }
    *num_walls = (new_wall_count < MAX_WALLS) ? new_wall_count : MAX_WALLS;

    // Comprehensive logging summary
    char log_msg[512];
    snprintf(log_msg, sizeof(log_msg), "SANITIZATION COMPLETE: Processed %d walls -> %d trimmed cells, %d walls kept intact, %d new segments created, final count: %d walls",
            walls_processed, cells_trimmed, walls_kept_intact, wall_segments_created, *num_walls);
    write_to_log(log_msg);

    if (new_wall_count > MAX_WALLS) {
        char log_msg2[256];
        snprintf(log_msg2, sizeof(log_msg2), "WARNING: Wall trimming created %d segments but MAX_WALLS limit is %d - some segments were truncated",
                new_wall_count, MAX_WALLS);
        write_to_log(log_msg2);
    }
}